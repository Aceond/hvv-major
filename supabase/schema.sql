...
-- 单图比分 upsert（PWA 战绩自动导入用）：按 (match_id, map_count) 覆盖/插入一张图的比分，
-- 并根据 match_maps 全量重算总比分与胜者（BO3 逐图导入互不覆盖，不会变成 6 局）。
...
declare
  ...
  v_stage_id uuid;
  v_group_id uuid;
  -- 旧值是否为「空」（null / 空串 / 空数组）：空才允许被本次新值填上
...
-- 匿名不含 steam_id；authenticated 含 steam_id：供「PWA 战绩自动导入」按 Steam64 位 ID 自动匹配双方队员（队长/管理员在战绩录入页读取队员 steam_id）
grant select (id, username, nickname, pw_username, highest_rank, highest_rating, role) on public.profiles to anon;
grant select (id, username, email, nickname, pw_username, role, highest_rank, highest_rating, account_status, created_at, steam_id) on public.profiles to authenticated;
