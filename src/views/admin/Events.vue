...
/** 删除赛事：级联清理该赛事报名/战队/赛程/冠军记录 */
async function removeEvent(row: EventItem) {
  try {
    await ElMessageBox.confirm(
      `确认删除赛事「${row.name}」？将同时删除该赛事的报名、战队、赛程与冠军记录，且不可恢复。`,
      '删除赛事',
...
<template>
  <div>
    ...
    <el-alert
      ...
      class="tip"
      title="系列赛事"
      description="HVV Major 一届一届持续举办（当前第十一届）。发布新赛事后，选手在「个人注册」中按赛事报名，报名中的赛事会展示在前台「赛事」入口。往届冠军在左侧「往届冠军」入口按组别录入。"
    />
