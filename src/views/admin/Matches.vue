...
async function generateAuto() {
  ...
  if (autoForm.mode === 'playoff') {
    ...
    pairs = seedPairs(ranks.slice(0, N).map((r) => r.teamId))
  } else {
    ...
    const winners = pr.filter((r) => pm.some((m) => m.winner_id === r.teamId))
    if (winners.length < M) {
      ...
      <el-form label-width="110px">
        ...
        <el-form-item>
          ...
          <span class="mode-hint">
            {{ autoForm.mode === 'playoff' ? '突围赛：排位第 N+1 名起取 K 支高低配对打，胜者进入淘汰赛' : autoForm.mode === 'knockoutN' ? '淘汰赛：排位赛前 N 名高低配对打（N 需为偶数）' : '淘汰赛：排位赛前 N 名 + 突围赛前 M 名胜者，混合高低配对打' }}
          </span>
        </el-form-item>
      </el-form>
